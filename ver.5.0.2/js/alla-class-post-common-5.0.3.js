...
$(document).ready(function () {
  ...
  // 이미지에 마우스 호버 시 커서를 포인터로 변경
  $('.allaGuideDiv img').hover(
    function () {
  ...
  // 정답표 "-" 기호 표시
  $('div.allaAnswerTableDiv table tr td:nth-of-type(4) img').attr('src', 'https://tistory3.daumcdn.net/tistory/4700529/skin/images/answer-none.png');

  // 정답표 "-" 기호 표시 (이미지 → 엔티티기호)
  // $("div.allaAnswerTableDiv table tr td:nth-of-type(4)").html("가");

  // 정답표 "바로가기" 기호 표시
  $('div.allaAnswerTableDiv table tr td:nth-of-type(5) img').attr('src', 'https://tistory3.daumcdn.net/tistory/4700529/skin/images/answer-shortcut.png');

  // 정답표 "바로가기" 기호 표시 (이미지 → 엔티티기호)
  // $("div.allaAnswerTableDiv table tr td:nth-of-type(5)").html("나");
    ...
    answerTableNumber.each(function () {
      ...
      if ($(this).text() == questionNumber) {
        ...
        if ($(this).next('td').text() == $(this).next().next('td').text()) {
          ...
          $(this).next().next().next().find('img').attr('src', 'https://tistory3.daumcdn.net/tistory/4700529/skin/images/answer-t.png');
          // (조건문) 정답이 틀렸다면
        } else if ($(this).next('td').text() != $(this).next().next('td').text()) {
          if ($.isNumeric($(this).next('td').text())) {
            $(this).next().next().next().find('img').attr('src', 'https://tistory2.daumcdn.net/tistory/4700529/skin/images/answer-x.png');
          } else {
    ...
    answerNumber.each(function () {
      ...
      if ($(this).text() == questionNumber.html()) {
        ...
        if ($(this).next().next().text() == '') {
          // $(questionNumber).parents('.allaBasicTbl').css('background-image', 'url(https://tistory3.daumcdn.net/tistory/4700529/skin/images/check-V.png)');
          ...
          $(questionNumber).parents('.allaBasicTbl').css('background-image', 'url(https://tistory3.daumcdn.net/tistory/4700529/skin/images/check-T.png)');
          $(questionNumber).parents('.allaBasicTbl').css('background-repeat', 'no-repeat');
          $(questionNumber).parents('.allaBasicTbl').css('background-size', '7.0em');
          $(questionNumber).parents('.allaBasicTbl').css('background-position', 'top left');
          // (조건문) 답안 선택이 "정답" 일 때
        } else if ($(this).next().next().text() == $(this).next().text()) {
          $(questionNumber).parents('.allaBasicTbl').css('background-image', 'url(https://tistory1.daumcdn.net/tistory/4700529/skin/images/check-O.png)');
          $(questionNumber).parents('.allaBasicTbl').css('background-repeat', 'no-repeat');
          $(questionNumber).parents('.allaBasicTbl').css('background-size', '7.0em');
          $(questionNumber).parents('.allaBasicTbl').css('background-position', 'top left');
          // (조건문) 답안이 틀렸을 때
          ...
          if ($.isNumeric($(this).next().text())) {
            $(questionNumber).parents('.allaBasicTbl').css('background-image', 'url(https://tistory2.daumcdn.net/tistory/4700529/skin/images/check-X.png)');
            $(questionNumber).parents('.allaBasicTbl').css('background-repeat', 'no-repeat');
            $(questionNumber).parents('.allaBasicTbl').css('background-size', '7.0em');
            $(questionNumber).parents('.allaBasicTbl').css('background-position', 'top left');
            // (조건문) 정답이 숫자가 아닐 때
  ...
  var divOtherExamDiv = $('div.allaOtherExamDiv');

  divOtherExamDiv.html(`
<a href="https://allaclass.tistory.com/tag/${examTitle}%20${examKind} target="_top">
  <div>
    <span class="allaOtherTitle">'${examTitle} ${examKind}'</span> 타년도 모의고사
  </div>
  <div>
    방송대 ${examTitle} ${examKind} 2019년도 / 올에이클래스 모의고사<br>
    방송대 ${examTitle} ${examKind} 2018년도 / 올에이클래스 모의고사<br>
    방송대 ${examTitle} ${examKind} 2017년도 / 올에이클래스 모의고사<br>
    방송대 ${examTitle} ${examKind} 2016년도 / 올에이클래스 모의고사<br>
    방송대 ${examTitle} ${examKind} 2015년도 / 올에이클래스 모의고사<br>
    방송대 ${examTitle} ${examKind} 2014년도 / 올에이클래스 모의고사<br>
    방송대 ${examTitle} ${examKind} 2013년도 / 올에이클래스 모의고사<br>
  </div>
</a>
  `);

  // divOtherExamDiv.html(
  //   '<a href="https://allaclass.tistory.com/tag/' +
  //     examTitle +
  //     '%20' +
  //     examKind +
  //     '" target="_top"><div><span class="allaOtherTitle">\'' +
  //     examTitle +
  //     ' ' +
  //     examKind +
  //     "'</span> 타년도 모의고사</div><div>방송대 " +
  //     examTitle +
  //     ' ' +
  //     examKind +
  //     ' 2019년도 / 올에이클래스 모의고사<br>방송대 ' +
  //     examTitle +
  //     ' ' +
  //     examKind +
  //     ' 2018년도 / 올에이클래스 모의고사<br>방송대 ' +
  //     examTitle +
  //     ' ' +
  //     examKind +
  //     ' 2017년도 / 올에이클래스 모의고사<br>방송대 ' +
  //     examTitle +
  //     ' ' +
  //     examKind +
  //     ' 2016년도 / 올에이클래스 모의고사<br>방송대 ' +
  //     examTitle +
  //     ' ' +
  //     examKind +
  //     ' 2015년도 / 올에이클래스 모의고사<br>방송대 ' +
  //     examTitle +
  //     ' ' +
  //     examKind +
  //     ' 2014년도 / 올에이클래스 모의고사<br>방송대 ' +
  //     examTitle +
  //     ' ' +
  //     examKind +
  //     ' 2013년도 / 올에이클래스 모의고사<br></div></a>'
  // );
});
